function [fnpt,grid_maps,all_points]=DarkBox_fits(image,border,minDist,brightspots,boxwidth,FinalPoints,segmentnumber,outname)
...
%% calculate the integrated intensity for background areas
pts=cnptB;
tmpBG=[];
for ff=1:size(pts,1)
    pc2=zeros(1,6);
...
set(f,'position',[696 29 637 952]);
%imagesc(image);
imshow(image,'DisplayRange',[mean(quantile(image,0.80)) mean(quantile(image,0.99))]); hold on;
hold on;
...
saveas(gcf, fullfile(dataout,[outname num2str(segmentnumber) '_Negativespace']), 'svg');hold off;
close all;
end
