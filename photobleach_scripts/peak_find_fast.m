function  [cent, varargout]=peak_find_fast(d, thres, filt ,edg, res, fid)
...
if (nargin < 2)
    thres = max(mean(d));%(max([min(max(d,[],1))  min(max(d,[],2))])); changed threshold to remove things that were below the mean
end

if (nargin < 3)
    filt = (fspecial('gaussian', 6,1)); %if needed modify the filter according to the expected peaks sizes
end
